...
class TeRDy(TKBCModel):
    def __init__(
            self, sizes: Tuple[int, int, int, int], rank: int,
            no_time_emb=False, alpha: float = 10.0, init_size: float = 1e-2
    ):
    ...
    def loss_freq(self, low_freq, high_freq):
        ...
        total_loss = (self.weight_separation * separation_loss \
                    + self.weight_high_freq_intensity * high_freq_intensity_loss) * \
                    (1 / (high_freq.shape[0] * high_freq.shape[0]))
        return total_loss
    ...
    def forward(self, x):
        ...
        loss_f = self.loss_freq(low_freq, high_freq)
